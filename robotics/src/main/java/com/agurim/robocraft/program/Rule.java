...
/**
 * One line of the program: {@code WHEN <source> <op> <rhs> THEN <target> <verb> [arg]}.
 *
...
public record Rule(String source, Op op, Operand rhs, String target, Verb verb, Operand arg) {
    ...
    /** The rule as the student reads it, e.g. "WHEN S1 < 7 THEN A1 ON". */
    public String text() {
        String head = always() ? ALWAYS : "WHEN " + source + " " + op.symbol() + " " + rhs.label();
